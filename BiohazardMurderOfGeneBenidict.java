...
public class BiohazardMurderOfGeneBenidict	//main class
{
	...
	{
		JFrame frame = new JFrame("Biohazard: The Murder of Gene Benidict");	//Creates JFrame
...
class BiohazardMurderOfGeneBenidictHolder extends JPanel
{
	...
	public BiohazardMurderOfGeneBenidictHolder()	
	{
		...
		firstCard title = new firstCard(this, cards); //adding all classes to card layout
		...
		wrongScreen wrong = new wrongScreen(this, cards);
		add(title, "title");
		add(report, "report");
		...
		add(correct, "correct");
		add(wrong, "wrong");
		showClueVid = true; //initializing variables
		startClip = darkenScreenPanel3 = correct1 = correct2 = correct3 = correct4 = correct5 = correct6 = correct7 = false;	//Initializes all the variables
	...
	public class SoundPlayer //throughout the code helps play sound effects
	{		
		public void playSound(String soundFilePath)
		{
			try
			{
				File soundFile = new File(soundFilePath); //takes in sound
	...
	public void readFile()
	{
		...
		{
			System.err.printf("\nERROR: Cannot find/open file %s. ", inFileName);	//error message
			System.exit(1);
		}
		while(input.hasNextLine()) //store the entire text file in string file
			file += input.nextLine() + "\n";
	}
	class firstCard extends JPanel implements MouseListener, MouseMotionListener, FocusListener	//Displays the title screen
	{
		...
		int startX, startY, startW, startH, targetW, targetH, clickOffset, repeat;
		...
		{
			addMouseListener(this); //adding listeners
			addMouseMotionListener(this);
			addFocusListener(this);
			
			panelCards = panelCardsIn; //initializing variables
			cards = cardsIn;
		...
		public void loadVTFont() //creates font
		{
			try
			{
		...
		public void setupNameField() //makes the text field for user to enter the name
		{
			setLayout(null);
		...
		public void blackScreen() //initializes timers and variables, also starting the black timer
		{
			...
			black = new Timer(4500, tbh);
			showTitle = new Timer(2000, stth);
			checkTime = new Timer(3000, ctth);
			black.start();
		}
		public void retrieveImage() //gets images
		{
			try
			{
		...
		public void startHoverAnimation() //starts hover animation, expanding image
		{
			hoverTimerHandler hth = new hoverTimerHandler();
			hoverTimer = new Timer(30, hth);
			hoverTimer.start();
		}
		class hoverTimerHandler implements ActionListener //expands and contracts the image
		{
			public void actionPerformed(ActionEvent e)	
			{
				...
				{
					if(Math.abs(scale - hoverScale) > tolerance) //tolerance is max it can go
		...
		class timerBlackHandler implements ActionListener
		{
			public void actionPerformed(ActionEvent e)
			{
				changeBlack = true; //drawing black screen
				repaint();
				black.stop();
				showTitle.start();
				
				try //starts playing the audio clip that lasts for most of the game
				{
					File soundFile = new File("Music.wav");
					AudioInputStream audioStream = AudioSystem.getAudioInputStream(soundFile);
					mainClip = AudioSystem.getClip();
					mainClip.open(audioStream);
					mainClip.start(); //play sound
					mainClip.loop(100);
				}
		...
		class fadeTimerHandler implements ActionListener //does the fading for the start button
		{
			public void actionPerformed(ActionEvent e)
			{
				if(!startReady)
		...
		public void startFadeIn() //starts the slow fade in of the button
		{
		...
		public void paintComponent(Graphics g)
		{
			...
			if(repeat == 0)
				soundPlayer.playSound("sirens.wav");	//plays sound
			
			repeat++;
			
			if(changeBlack)
				g.fillRect(0, 0, 1300, 800);	//draws black screen
			
			if(showImage && changeBlack)
			{
				Graphics2D g2d = (Graphics2D)(g);
				g2d.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, titleTransparency));	//draws title image
				g2d.drawImage(titleImage, 250, 20, 800, 600, this);	//draws title image
				...
				if(startReady)
				{
					...
					if(clickingStart)
					{
						...
						g2d.fillRect(drawX, drawY + 26, targetW, targetH);
					}
				}
			}
		}
	...
	class secondCard extends JPanel 	//Forensic Report Panel
	{
		BiohazardMurderOfGeneBenidictHolder panelCards;
		CardLayout cards;
		thirdCard darkScreen;
		...
		public secondCard(BiohazardMurderOfGeneBenidictHolder panelCardsIn, CardLayout cardsIn)
		{
			...
			darkScreen = new thirdCard(panelCards, cards);
			ImageIcon originalIcon = new ImageIcon("coronersReport.png"); //load forensics image
			...
			try
			{
				fullImage = ImageIO.read(new File("beginMurderButtons.png")); //load in murder buttons
			}
			catch (IOException e)
			{
				System.err.println("\n\n\nERROR IN RETRIEVING IMAGE\n\n\n"); //error trap
			...
			JLayeredPane layeredPane = new JLayeredPane(); //add the forensics image to the layered pane
			...
			JButton startGameButton = new JButton(scaledReturnIcon); //create the button with the cropped image
			int originalWidth = 400;
			int originalHeight = 65;
			int hoverWidth = 430;
			int hoverHeight = 75;
			class startGameButtonHandler implements ActionListener //returns user to specified question they exited from
			{
				public void actionPerformed(ActionEvent e)
				{
					...
					else if(goingBacktoClue.equalsIgnoreCase("question4"))
					{
			...
			class buttonHandler implements MouseListener
			{
				...
				hoverTimerHandler hth = new hoverTimerHandler();	//hover effect
				public void mouseEntered(MouseEvent e)	
				{
				...
				class shrinkTimerHandler implements ActionListener		//shrink effect
				{
					int currentWidth = startGameButton.getWidth();
					int currentHeight = startGameButton.getHeight();
					public void actionPerformed(ActionEvent evt)
	...
	class thirdCard extends JPanel implements MouseListener, MouseMotionListener	//Tutorial Panel
	{
		BiohazardMurderOfGeneBenidictHolder panelCards;
		CardLayout cards;
		...
		public thirdCard(BiohazardMurderOfGeneBenidictHolder panelCardsIn, CardLayout cardsIn)	// constructor
		{
			panelCards = panelCardsIn;	//initializing variables
			cards = cardsIn;	//sets up the card layout
			showQuestions = showDarkScreen = hoveredOnForensic = clickedOnForensic = secondDarkScreen = false;	//boolean variables
			darkScreenTimerHandler dsth = new darkScreenTimerHandler();	//dark screen timer
			secondDarkTimerHandler sdth = new secondDarkTimerHandler(); //second dark screen timer
			secondDarkTimer = new Timer(3500, sdth);//sets up the second dark screen timer
			darkScreenTimer = new Timer(1000, dsth);//sets up the dark screen timer
			darkScreenTimer.setRepeats(false);//sets up the dark screen timer
			secondDarkTimer.setRepeats(false); //sets up the second dark screen timer
	...
	class fourthCard extends JPanel implements MouseListener, MouseMotionListener	//Clue Board without Gif
	{
		BiohazardMurderOfGeneBenidictHolder panelCards;
		CardLayout cards;
		...
		{
			addMouseListener(this);	//addMouse Listener
		...
		public void paintComponent(Graphics g)
		{
			...
			else if(blackScreen)	//black screen
			{
				g.setColor(Color.BLACK);
			...
			//highlighting based on correct answers
			if(correct1)
			{
		...
		public void mouseClicked(MouseEvent e)
		{
			...
			int x = e.getX();
			int y = e.getY();
			if(x >= 115 && x <= 224 && y >= 189 && y <= 334 && !correct1 && !wrong1)
			{
			...
			else if (x >= 563 && x <= 685 && y >= 137 && y <= 257&& !correct2&& !wrong2)
			{
				cards.show(panelCards, "question1");
				AnsweringQ = 2;
			}
			else if (x >= 909 && x <= 1125 && y >= 142 && y <= 297&& !correct3&& !wrong3)
			{
				cards.show(panelCards, "question1");
				AnsweringQ = 3;
			}
			else if (x >= 158 && x <= 316 && y >= 355 && y <= 525&& !correct4&& !wrong4)
			{
				cards.show(panelCards, "question1");
				AnsweringQ = 4;
			}
			else if (x >= 78 && y >= 540 && x <= 192 && y <= 693&& !correct5&& !wrong5)
			{
				cards.show(panelCards, "question1");
				AnsweringQ = 5;
			}
			else if (x >= 210 && y >= 565 && x <= 392 && y <= 759&& !correct6&&!wrong6)
			{
				cards.show(panelCards, "question1");
				AnsweringQ = 6;
			}
			else if (x >= 981 && y >= 556 && x <= 1131 && y <= 655 && !correct7&&!wrong7)
			{
				cards.show(panelCards, "question1");
				AnsweringQ = 7;
			}
	...
	class question1 extends JPanel implements MouseListener
	{
		...
		class centerPanel extends JPanel
		{
			...
			public void paintComponent(Graphics g)
			{
				...
				g.drawImage(hint, 50, 550, 50, 60, this);
				if(darkenForensic)
				{
					g.setColor(new Color(0, 0, 0, 80));
					g.fillRect(50, 550, 50, 60);
				}
			}
		}
	...
	class question2 extends JPanel implements MouseListener, MouseMotionListener	//2nd question
	{
		boolean correct;
		boolean darkenForensic;
		...
		class centerPanel extends JPanel
		{
			...
			public void paintComponent(Graphics g)
			{
				...
				g.drawImage(questionBase, 50, -20, 920, 300, this);
	...
	class question4 extends JPanel implements MouseListener
	{
		...
		class centerPanel extends JPanel
		{
			...
			public void paintComponent(Graphics g)
			{
				...
				for(int i = 0; i < indOptions.length; i++)
					g.drawString(indOptions[i], 170, 180  + (i * 45));
				Image hint = new ImageIcon("forensicButton.png").getImage();
				g.drawImage(hint, 30, 695, 50, 60, this);
				if(darkenForensic)
				{
					g.setColor(new Color(0, 0, 0, 80));
					g.fillRect(30, 550, 50, 60);
		...
		public void mouseReleased(MouseEvent e)
		{
			...
			else if(x >= 30 && x <= 100 && y >= 695 && y <= 755)
			{
				cards.show(panelCards, "report");
				lowerTimer.stop();
				countdownTimer.stop();
	...
	public class HighScoresPanel extends JPanel implements MouseListener, MouseMotionListener
	{
		CardLayout cards;
		BiohazardMurderOfGeneBenidictHolder panelCards;
		
		public HighScoresPanel(BiohazardMurderOfGeneBenidictHolder panelCardsIn, CardLayout cardsIn)
		{
			addMouseListener(this);
			addMouseMotionListener(this);
			requestFocusInWindow();
			...
			backButton.setFont(font.deriveFont(30f));
			backButtonHandler bbh = new backButtonHandler();
			backButton.addActionListener(bbh);
			backButton.setBounds(550, 600, 200, 50); // Set x, y, width, height
			add(backButton);
			...
			if(gameCorrect)
				textArea.append("\nYour Result: Found the Killer!\n");
			else
				textArea.append("\nYour Result: Did not find the Killer.\n");
		...
		public void paintComponent(Graphics g)
		{
			...
			Image returnButton = new ImageIcon("returnButton.png").getImage();
			g.drawImage(returnButton, 550, 600, 200, 50, this);
		}
		@Override
		public void mouseDragged(MouseEvent e) {
	...
	class correctScreen extends JPanel implements MouseListener, MouseMotionListener
	{
		...
		Timer stopClipTimer;
		int repeats, naviWidth, naviHeight;
		float fade = 0.0f;
		Timer fadeInTimer;
		...
		public void paintComponent(Graphics g)
		{
			...
			if(repeats == 0)
			{
				stopClipTimer.start();
				mainClip.stop();
				soundPlayer.playSound("Taylor Swift - no body, no crime (Official Lyric Video) ft. HAIM.wav");
			}
			repeats++;
			if(drawImage)
			{
		...
		public void mouseClicked(MouseEvent e)
		{
			...
			{
				HighScoresPanel highScores = new HighScoresPanel(panelCards, cards);
				BiohazardMurderOfGeneBenidictHolder panelCards = (BiohazardMurderOfGeneBenidictHolder) getParent();
				panelCards.add(highScores, "highScores");
	...
	class wrongScreen extends JPanel implements MouseListener, MouseMotionListener
	{
		BiohazardMurderOfGeneBenidictHolder panelCards;
		CardLayout cards;
		Image clueClip, clueBG, navigation;
		...
		float fade = 0.0f;
		Timer fadeInTimer;
		public wrongScreen(BiohazardMurderOfGeneBenidictHolder panelCardsIn, CardLayout cardsIn)
		{
			panelCards = panelCardsIn;
			cards = cardsIn;
			addMouseListener(this);
			...
			stopClipTimerHandler scth = new stopClipTimerHandler();
			clueClip = storeGif.getImage();
			stopClipTimer = new Timer(9000, scth);
			drawImage = rightHover = leftHover = false;
			repeats = 0;
		}
		public void retreiveImage()
		{
			try
			{
				clueBG = ImageIO.read(new File("loseBG.png"));
				navigation = ImageIO.read(new File("navigation.png"));
			}
			catch(IOException e)
			{
		...
		public void paintComponent(Graphics g)
		{
			...
			if(repeats == 0)
			{
				stopClipTimer.start();
				mainClip.stop();
				soundPlayer.playSound("Castles Crumbling (Taylors Version) (From The Vault) (Lyric Video).wav");
			}
			repeats++;
			if(drawImage)
			{
