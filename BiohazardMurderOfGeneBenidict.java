...
class BiohazardMurderOfGeneBenidictHolder extends JPanel
{
	...
	class thirdCard extends JPanel implements MouseListener, MouseMotionListener
	{
		...
		public void paintComponent(Graphics g)
		{
			...
			else if(clueClicks[6])
				g2d.fillRect(1075, 351, 135, 76);
			else if(clueClicks[7])
			{
				int[] sevenX = {88, 67, 182, 202};
				int[] sevenY = {537, 680, 696, 551};
				g2d.fillPolygon(sevenX, sevenY, 4);
			}
			else if(clueClicks[8])
			{
				
			} 
			else if(clueClicks[9])
			{
				int[] nineX = {450,562, 533, 423};
				int[] nineY = {589, 613, 730, 759};
				g2d.fillPolygon(nineX, nineY, 4);
			}
			else if(clueClicks[10])
			{
				int[] tenX = {755, 881,876, 746};
				int[] tenY = {647, 667, 738, 717};
				g2d.fillPolygon(tenX, tenY, 4);
			}
			else if(clueClicks[11])
			{

				g2d.fillRect(981, 556, 150, 99);
